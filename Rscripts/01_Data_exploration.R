...
col_types_list <- cols_only(posID = "d", individual = "d", plot = col_factor(levels=c(1:12)), animal = "d", 
                            ...
                            height = "d", stem_diam = "d",
			                      ...
			                      cone_vol = "d", cone_vol_raster = "d", cone_vol_neigh = "d"
                            )
...
data[,11:50] <- lapply(data[,11:50], as.numeric) #setting all neccessary parameters to class numeric
...
dat <- data %>%
  ...
  select(posID, individual, plot, matID, patID, famID, mat_group, treatment,
         ...
         height, stem_diam, germ_neigh, cone_vol, cone_vol_neigh)
...
#Calculating Relative Fitness for Each Individual #LAST ENDED HERE
dat <- dat %>%
  group_by(treatment) %>%
  mutate(relative = seed_pods / 4.99)
  
dat <- dat %>%
  mutate(relative = ifelse(treatment == "A", seed_pods/4.99, seed_pods/9.27))
...
#Graphs 2 - Histogram Distribution for Continuous Traits (INCLUDING germ = 0) ####
...
#Graphs 3 - Density Plots for Distribution for Continuous Traits (EXCLUDING germ = 0) ####
...
mu.density <- dat %>%
  filter(!germ==0) %>%
  group_by(treatment) %>%
  summarise(grp.mean=mean(germ_neigh))
dat %>% #neighbour density by germination
  filter(!germ==0) %>%
  ggplot(aes(x=germ_neigh, fill=treatment, color=treatment)) +
  geom_histogram(alpha=0.3, position="identity", binwidth=1) + #histogram because integers
  geom_vline(data=mu.density, aes(xintercept=grp.mean, color=treatment), linetype="dashed", size=1) +
  scale_x_continuous(breaks=seq(0, 20, 2)) +
  scale_color_manual("Treatment", values=c("dodgerblue2", "tomato2"), labels=c("Ambient", "Heated")) +
  scale_fill_manual(values=c("dodgerblue2", "tomato2")) +
  labs(x="Number of Leaves", y="Frequency")  + 
...
mu.relative <- dat %>%
  #filter(!germ==0) %>%
  group_by(treatment) %>%
  summarise(grp.mean=mean(relative))
dat %>% #relative fitness
  #filter(!germ==0) %>%
  ggplot(aes(x=relative, fill=treatment, color=treatment)) +
  geom_histogram(alpha=0.3, position="identity", bins=100, binwidth=1) +
  geom_vline(data=mu.relative, aes(xintercept=grp.mean, color=treatment), linetype="dashed", size=1) +
  scale_x_continuous(breaks=seq(0, 40, 2)) +
  scale_color_manual("Treatment", values=c("dodgerblue2", "tomato2"), labels=c("Ambient", "Heated")) +
  scale_fill_manual(values=c("dodgerblue2", "tomato2")) +
  labs(x="Relative Fitness", y="Frequency")  + 
  theme_bw() + theme(legend.position="none") + 
  theme(panel.border = element_blank(), panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  theme(axis.line = element_line(colour = "black", size = 1.5), 
        axis.ticks=element_line(size=1.5), axis.ticks.length=unit(0.2, "cm"))

#Graphs 4 - Trait distribution by PLOT for continuous Traits (leaf, flwr_clstr, seed_pods) ####
dat %>% #leaves
...
#Graphs 5 - Trait distribution by FAMILY/SIBSHIP for binary traits (germ, flower, seed) ####
...
#Graphs 6 - Trait distributions by FAMILY/SIBSHIP for continuous traits (leaf, flwr_clstr, seed_pods) ####
...
#Graphs 7 - Trait distribution by FAMILY/SIBSHIP for BINARY traits (germ, flower, seed) ####
...
#Graphs 8 - Trait distributions by FAMILY/SIBSHIP for CONTINUOUS traits (leaf, flwr_clstr, seed_pods) ####
...
#Graphs 9 - Maternal Correlations for BINARY traits (germ, flower, seed) ####

#Germination (or Overwintering Survival)
...
#Flowering Success - or Summer Survival
...
#Graphs 10 - Maternal Correlations for CONTINUOUS traits (leaf, flwr_clstr, seed_pods) ####
...
pods_mat <- dat %>% #seed pods
 # filter(germ==1) %>% remove # just seed_pod
  group_by(treatment, famID, matID) %>%
  summarise(pods_avg=mean(seed_pods), n=n(), sd=sd(seed_pods), se=(sd/(sqrt(n)))) %>%
...
pods_mat %>% #Note: 12 removed values because no paired maternal group
  ...
  scale_color_manual("Treatment", values=c("steelblue2", "tomato2"), labels = c("Ambient", "Heated")) +
  labs(x="Mean Lifetime Fitness - Maternal Subgroup A", y="Mean Lifetime Fitness - Maternal Subgroup B")  +
  ggtitle("C)") +
  theme_bw() + theme(legend.position="none") + 
  theme(panel.border = element_blank(), panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  theme(axis.line = element_line(colour = "black", size = 1.5), 
...
#Graphs 11 - Maternal Correlations Fitness Component Traits ####
...
surv_mat %>% #Note: 12 removed values because no paired maternal group
  ...
  scale_x_continuous(limits=c(0,1)) +
  ggtitle(label="A)") +
  labs(x="Mean Survival to Flowering - Maternal Subgroup A", y="Mean Survival to Flowering - Maternal Subgroup B")  +
  theme_bw() + theme(legend.position="none") + 
  theme(panel.border = element_blank(), panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  theme(axis.line = element_line(colour = "black", size = 1.5), 
        axis.ticks=element_line(size=1), axis.ticks.length=unit(0.2, "cm"))
...
fecundity_mat %>% #Note: 12 removed values because no paired maternal group
  ggplot(aes(x=pods_avg_A, y=pods_avg_B, group=treatment, color=treatment)) +
  ...
  geom_errorbarh(aes(y=pods_avg_B, xmin=(pods_avg_A-se_A), xmax=(pods_avg_A+se_A), alpha = 0.5)) +
  scale_color_manual("Treatment", values=c("steelblue2", "tomato2"), labels = c("Ambient", "Heated")) +
  labs(x="Mean Fecundity - Maternal Subgroup A", y="Mean Fecundity - Maternal Subgroup B")  +
  ggtitle("B)") +
  theme_bw() + theme(legend.position="none") + 
  theme(panel.border = element_blank(), panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  theme(axis.line = element_line(colour = "black", size = 1.5), 
...
#Graphs 12 - Genetic Correlations between Select Traits and Seed Pod Number (Fitness) ####
...
#Seed Pod Number against Germination Time - Individual Level
dat %>%
  filter(!germ==0) %>%
  ggplot(aes(x=germ_census, y=seed_pods, color=treatment)) +
  geom_point(aes(color=treatment), alpha=0.5) +
  geom_smooth(se=F, method="glm") +
  scale_color_manual("Treatment", values=c("dodgerblue2", "tomato2"), labels=c("Ambient", "Heated")) +
  labs(x="Germination Time (census #)", y="Number of Seed Pods") +
  ggtitle("A)") + 
  theme_bw() + theme(legend.position="none") + 
  theme(panel.border = element_blank(), panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  theme(axis.line = element_line(colour = "black", size = 1.5), 
...
#Seed Pod Number against Germination Time - Family Level
dat %>%
  filter(!germ==0) %>%
  group_by(treatment, famID) %>%
...
#Seed Pod Number against Flowering Time - Individual Level
dat %>%
  filter(!germ==0) %>%
  ggplot(aes(x=flwr_census, y=seed_pods, color=treatment)) +
  geom_point(aes(color=treatment), alpha=0.5) +
  geom_smooth(se=F, method="glm") +
  scale_color_manual("Treatment", values=c("dodgerblue2", "tomato2"), labels=c("Ambient", "Heated")) +
  labs(x="Flowering Time (census #)", y="Number of Seed Pods") +
  ggtitle("A)") + 
  theme_bw() + theme(legend.position="none") + 
  theme(panel.border = element_blank(), panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  theme(axis.line = element_line(colour = "black", size = 1.5), 
...
#Seed Pod Number against Flowering Time - Family Level
dat %>%
  filter(!germ==0) %>%
  group_by(treatment, famID) %>%
...
#Graphs 13 - Life-time Fitness Summary by Proportion of Total Plants ####
sum.fit <- dat %>%
  group_by(treatment) %>%
...
#Graphs 14 - Density plots using FAMILY MEANS for fitness traits ####
...
fam.lifetime %>% #lifetime fitness (seed pods)
  ...
  theme(axis.line = element_line(colour = "black", size = 1.5), 
        axis.ticks=element_line(size=1.5), axis.ticks.length=unit(0.2, "cm"))  +
  theme(axis.text.x=element_text(colour = "black", hjust=0.5, vjust=0.5))
...
flwr_summary <- dat %>%
  group_by(treatment) %>%
  summarise(sum=sum(flower), n=n(), flwr_avg=sum/n)
flwr_fam %>% #flowering success of all plants
  ggplot(aes(x=flwr_by_fam, fill=treatment, color=treatment)) +
  geom_density(alpha=0.3) +
  geom_vline(data=flwr_summary, aes(xintercept=flwr_avg, color=treatment), linetype="dashed", size=1) +
  scale_color_manual("Treatment", values=c("dodgerblue2", "tomato2"), labels=c("Ambient", "Heated")) +
  scale_fill_manual(values=c("dodgerblue2", "tomato2")) +
  scale_x_continuous(limits=c(0,1)) +
...
fam.relative <- dat %>%
  filter(!germ==0) %>%
  group_by(treatment, famID) %>%
  summarise(fam.mean=mean(relative))
fam.relative %>% #seed pods
  ggplot(aes(x = fam.mean, fill = treatment, color = treatment)) + 
  geom_density(alpha=0.3) +
  geom_vline(data=mu.relative, aes(xintercept=grp.mean, color=treatment), linetype="dashed", size=1) +
  scale_color_manual("Treatment", values=c("dodgerblue2", "tomato2"), labels = c("Ambient", "Heated")) +
  ...
  labs(x="Mean Family Relative Fitness", y="Frequency")  +
  ggtitle("B)") +
  theme_bw() + theme(legend.position="none") + 
  theme(panel.border = element_blank(), panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  theme(axis.line = element_line(colour = "black", size = 1.5), 
...
## Graphs 15 - Evaluating Seed Pod as Proxy for Fitness ####
...
dat %>% 
  ...
  scale_fill_manual(values=c("dodgerblue2", "tomato2")) +
  #scale_x_continuous(limits=c(0,5)) +
  labs(x="Total Seed Pod Number per Plant", y="Seed Number per Plant")  +
  #ggtitle("B)") +
  theme_bw() + theme(legend.position="none") + 
  theme(panel.border = element_blank(), panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  theme(axis.line = element_line(colour = "black", size = 1.5), 
...
#### Pre-Analysis 1 - GLM + GLMM testing mean differences caused by TREATMENT ONLY for Continuous Traits ####
...
#Just testing random effects on family and nested maternal effects to see if MCMCglmm models are running correctly (getting low estimates of Va)
dat5.a <- dat5 %>% filter (treatment=="A")
dat5.h <- dat5 %>% filter (treatment=="H")
model5d.a <- lmer(dat=dat5.a, leaf ~ (1|famID) + (1|famID:matID), REML=FALSE)
model5d.h <- lmer(dat=dat5.h, leaf ~ (1|famID) + (1|famID:matID), REML=FALSE)
model5d <- lmer(dat=dat5, leaf ~ treatment + (1|famID) + (1|famID:matID), REML=TRUE)
summary(model5d)
...
#### Pre-Analysis 2 - LMM (Frequentist) models for Fitness Traits ####
...
#### Pre-Analysis 3 - Calculating Raw Estimate of Adaptive Potential ####
