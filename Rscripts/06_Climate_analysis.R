#### PROJECT: Climate observations (Data collected by KSR Weather Station at Koffler Scientific Reserve, King City, ON)
#### PURPOSE: Observe temperature and precipitation at study site vs collection site 
#### AUTHOR: Cameron So
...
climate <- climate %>% filter(!Month == "August")
...
temp_fig <- climate %>%
  ...
  scale_linetype_manual("", values=c("solid", "dotted"), labels=c("Experimental site (ON)", "Parental site (QC)")) +
  scale_color_manual("", values=c("gray40", "darkslategray3", "orange2"), labels=c("Daily average", "Minimum", "Maximum")) +
  #guides(color=guide_legend("Temperature"), linetype=FALSE) + #Remove colour guide
  labs(y="Temperature (°C)", tag = "A)") +
  scale_x_continuous("Month", labels=c("April", "May", "June", "July")) +
  theme_bw() + 
  theme(panel.border = element_blank(), panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
      ...
      legend.spacing.y = unit(0.0001, "mm"))
temp_fig
ggsave(plot = temp_fig, filename = "Routput/Figures/fig_2.tif", width = 12, height = 9.5, units ="cm")
...
precip_fig <- climate %>%
  ...
  geom_bar(color="black", position="dodge", stat="identity", size=0.5) +
  labs(x="Month", y="Monthly precipitation (mm)", tag = "B)") +
  scale_x_continuous("Month", breaks=seq(4,7,1), labels=c("April", "May", "June", "July")) + 
  scale_fill_manual("", values=c("forestgreen", "darkolivegreen3"), 
                    labels=c("Experimental site (ON)", "Parental site (QC)")) +
  ...
  theme(panel.border = element_blank(), panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        ...
        legend.spacing.y = unit(0.0001, "mm"))
precip_fig
ggsave(plot = precip_fig, filename = "Routput/Figures/fig_2b.tif", width = 12, height = 9.5, units = "cm")

fig_2 <- temp_fig + precip_fig
ggsave(plot = fig_2, filename = "Routput/Figures/fig_2.tif", width = 24, height = 9.5, units = "cm")
