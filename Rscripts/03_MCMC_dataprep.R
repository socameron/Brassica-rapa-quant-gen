...
dat <- dat %>%
  mutate(relative = ifelse(treatment == "A", seed_pods/4.99, seed_pods/9.26))
...
MCMC.2019<- dat %>%
  select(posID, individual, animal, plot, matID, patID, famID, mat_group, treatment, germ, flower, seed, leaf, flwr_clstr, seed_pods, seed_number, height, stem_diam, germ_neigh, germ_census, flwr_census, relative)
